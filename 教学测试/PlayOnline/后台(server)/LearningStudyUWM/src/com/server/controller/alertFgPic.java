...
//				FileGroup group = CommonUtils.toBean(map, FileGroup.class);
				FileGroup oldGroup = service.getFileGroupById(fileGroupId);
				String oldFileName = CommonUtils.getFileName(oldGroup.getImgSrc());
				String oldImgPath = CommonUtils.getProperties("config.properties", "deletePicDir") + File.separator;
//				FileGroup fileGroup = CommonUtils.toBean(map, FileGroup.class);
				...
				//操作数据库
				int result = service.alertFileGroupPic(params);
				if(result != 0){
